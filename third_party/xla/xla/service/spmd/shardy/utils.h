...
using ConsumeAxisRefsFn =
    std::function<void(mlir::ArrayRef<mlir::sdy::AxisRefAttr>)>;
using ForEachAxisRefListFn =
    std::function<void(ConsumeAxisRefsFn consumeAxisRefs)>;

// Return all axes or sub-axes in the `mesh`, such that sub-axes are derived
// from `forEachAxisRef` and sorted by their order in the mesh. For example,
...
mlir::SmallVector<mlir::sdy::AxisRefAttr> getOrderedAxisRefs(
    ForEachAxisRefListFn forEachAxisRefList, mlir::sdy::MeshAttr mesh);

// Same as the overload above, but takes a list of `axisRefs`directly.
mlir::SmallVector<mlir::sdy::AxisRefAttr> getOrderedAxisRefs(
    mlir::ArrayRef<mlir::sdy::AxisRefAttr> axisRefs, mlir::sdy::MeshAttr mesh);
