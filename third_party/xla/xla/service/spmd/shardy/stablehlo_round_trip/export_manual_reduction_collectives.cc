...
mlir::DenseIntElementsAttr getReplicaGroups(sdy::AllReduceOp op,
  ...
  for (AxisRefAttr axis : meshAxisRefs) {
  ...
  array.Reshape({numGroups * groupSize});
...
class AllReducePattern : public OpConversionPattern<sdy::AllReduceOp> {
 ...
 private:
  LogicalResult matchAndRewrite(
    ...
    // TODO(tomnatan): add unique channel id.
    ...
    auto newAllReduce = rewriter.replaceOpWithNewOp<stablehlo::AllReduceOp>(
        op, op.getType(), adaptor.getTensor(),
        getReplicaGroups(op, op.getOutSharding().getMesh(op), rewriter),
        stablehlo::ChannelHandleAttr::get(op->getContext(),
                                          /*channelId=*/1,
                                          /*channelType=*/0),
